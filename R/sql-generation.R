...
#' @export
sqlUpdateInTable <- function(con,table1, table2, set = NULL, by = NULL){
...
#' generate sql for an alter table statment adding a primary key
#' @export
sqlAlterTableWithPrimaryKey <- function(con,target.table, pk ){
  target.table.q <- DBI::dbQuoteIdentifier(con,target.table);
  pk.q <- sapply(pk,function(x) {print(x); DBI::dbQuoteIdentifier(con,as.character(x))});
  sql_alter_table <- DBI::SQL(
    paste0("ALTER TABLE ", target.table.q, "\n",
           "ADD PRIMARY KEY (", paste0(pk.q, collapse = ", "), ");"
    )
  )
}
